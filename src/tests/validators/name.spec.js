...
test('Name Validator: Ensure that name supplied is valid', t => {
  ...
  t.deepEqual(result, tester, 'The name objects should be the same')
  t.end()
})
...
test('Name Validator: Ensure that first name supplied breaks empty validation', t => {
  ...
  t.deepEqual(result.object, tester, 'The name objects should be the same')
  t.end()
})
...
test('Name Validator: Cause more than one validation error', t => {
  ...
  // console.log(result.error)
  const { error } = result
  t.equal(error.message, '"first" is not allowed to be empty', 'Validation error message should be correct')
  t.equal(error.path, 'first', 'Validation error property should be "first"')
