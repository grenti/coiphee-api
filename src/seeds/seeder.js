...
const CoiffeurSeeder = require('./coiffeur')
const ShoppeSeeder = require('./shoppe')
const ServiceCategorySeeder = require('./serviceCategory')
const ServiceSeeder = require('./service')
...
const coiffeur = async () => {
  try {
    ...
    const coiffeurs = []
    for (let i = 0; i < 500; i++) {
      coiffeurs.push(CoiffeurSeeder.build())
    }
...
const shoppe = async (services = [], coiffeurs = []) => {
  try {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      shoppes.push(ShoppeSeeder.build(slicedService.map(s => s._id), slicedCoiffeur.map(c => c._id)))
    }
...
const serviceCategory = async () => {
  try {
    ...
    for (let i = 0; i < 50; i++) {
      serviceCategories.push(ServiceCategorySeeder.build())
    }
...
const service = async (serviceCategories = []) => {
  try {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      services.push(ServiceSeeder.build(serviceCat._id))
    }
...
const question = async () => {
  try {
    const questions = []
    for (let i = 0; i < 20; i++) {
      questions.push(QuestionSeeder.build())
    }
